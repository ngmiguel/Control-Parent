...
public class AuthController {
    ...
    /**
     * Endpoint de debug pour tester la normalisation et la recherche de numéros
     * GET /api/auth/debug-telephone?numero=696091296
     */
    @GetMapping("/debug-telephone")
    public ResponseEntity<?> debugTelephone(@RequestParam String numero) {
        try {
            boolean existe = authService.estParentValide(numero);
            String message = String.format(
                "Numéro testé: %s\nParent trouvé: %s",
                numero,
                existe ? "OUI ✅" : "NON ❌"
            );
            return ResponseEntity.ok(new MessageResponse(message, existe));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
